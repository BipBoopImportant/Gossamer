...
class _ChatScreenState extends ConsumerState<ChatScreen> {
  ...
  Widget build(BuildContext context) {
    final thread = ref.watch(chatProvider).where((m) => m.core.sender == widget.ghostId || (m.core.isMe && (m.core.sender == "Me" || m.core.sender == widget.ghostId))).toList();
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return Align(
                    alignment: msg.core.isMe ? Alignment.centerRight : Alignment.centerLeft,
                    child: Container(
                      margin: const EdgeInsets.only(bottom: 12),
                      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                      constraints: const BoxConstraints(maxWidth: 260),
                      decoration: BoxDecoration(
                        color: msg.core.isMe ? const Color(0xFF6C63FF) : const Color(0xFF1A1A24),
                        borderRadius: BorderRadius.circular(16),
                      ),
                      child: Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          Flexible(child: Text(msg.core.text, style: const TextStyle(color: Colors.white))),
                          if (msg.core.isMe) ...[
                      ...
                      IconButton.filled(
                        onPressed: () {
                          if (_ctrl.text.trim().isNotEmpty) {
                            String dest = widget.ghostId.length == 64 ? widget.ghostId : ref.read(contactsProvider).value?.firstWhere((c) => c.alias == widget.ghostId, orElse: () => const Contact(pubkey: "00", alias: "")).pubkey ?? "00";
  ...
  Widget _buildStatusIcon(MessageStatus status) {
    switch (status) {
      case MessageStatus.sending:
        return const SizedBox(width: 12, height: 12, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white54));
      case MessageStatus.sent:
        return const Icon(UniconsLine.check, size: 16, color: Colors.white54);
      case MessageStatus.failed:
        return const Icon(UniconsLine.exclamation_triangle, size: 16, color: Colors.redAccent);
    }
  }
}

...
impl Database {
    ...
    // Look up a contact alias, or return the shortened key
    pub fn resolve_sender(&self, pubkey: &str) -> String {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare("SELECT alias FROM contacts WHERE pubkey = ?1").unwrap();
        let mut rows = stmt.query([pubkey]).unwrap();
        if let Ok(Some(row)) = rows.next() {
            return row.get(0).unwrap();
        }
        // Fallback: Truncate ID
        if pubkey.len() > 8 {
